...
def _parse_args():
    parser = argparse.ArgumentParser(description="Extract entities from chunks.jsonl.")
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        ...
        help="Output JSONL path (default: entities.jsonl).",
    )
    parser.add_argument(
        "--signals",
        action="store_true",
...
def _build_prompt(text):
    return (
        ...
        '{ "items": [ { "entity": string, "type": string, "quote": string, "confidence": float } ] }\n'
        "Use an empty list if no entities are found. "
...
def main():
    ...
    extractor_version = f"entities:v1:{args.provider}:{model}"
