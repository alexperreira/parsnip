...
def _parse_args():
    parser = argparse.ArgumentParser(description="Load conversations JSONL into SQLite.")
    parser.add_argument(
        "--input",
...
def build_load_conversations(input_path, db_path, overwrite=False):
        ...
        for raw_line in handle:
            ...
            for item in items:
                ...
                if not isinstance(item, dict):
                    ...
                    continue
                speaker = as_clean_text(item.get("speaker"))
                if not speaker:
                    summary["invalid_item_shape"] += 1
                    summary["rows_skipped"] += 1
                    continue
    ...
    mark_loader_run(conn, "conversations")
    conn.commit()
    conn.close()
...
def main():
    args = _parse_args()
    summary = build_load_conversations(args.input, args.db, overwrite=args.overwrite)
    print("Load conversations summary")
