...
class CliRunHelpTest(unittest.TestCase):
    def test_run_help_includes_llm_provider_options(self):
        ...
        self.assertIn("--llm-openai-base", output)
        self.assertIn("--llm-gemini-base", output)
        self.assertIn("--llm-two-pass", output)
        self.assertIn("--llm-retry-small", output)
        self.assertIn("extract-text,chunk", output)
        self.assertIn("--triage-max-llm-", output)
        self.assertIn("--triage-ml-route", output)
